...
service HealthServer on ep {
    ...
    resource function registerPatient(grpc:Caller caller, Patient patient) {
        log:printInfo("Registration process begins");
        log:printInfo("Received patient information");
        log:printInfo("Patient ID: " + patient.id);
        log:printInfo("Patient name: " + patient.name);
        log:printInfo("Patient gender: " + patient.gender.toString());
        log:printInfo("Patient disease: " + patient.disease);

        grpc:Error? result = caller->send(true);
    ...
    resource function getPatientInfo(grpc:Caller caller, string patientId) {
        ...
        grpc:Error? result = caller->send(patient);
        if (result is grpc:Error) {
